...
const CustomTabs = () => (
  <Tabs.Root className="flex flex-col w-full lg:w-[900px] flex-wrap" defaultValue="tab1">
    <Tabs.List className="shrink-0 flex flex-wrap mb-14" aria-label="Manage your account">
      {tabs.map((tab) => (
        <Tabs.Trigger
          key={tab.id}
          className="bg-white uppercase px-5 h-[45px] flex-1 flex items-center justify-center text-xs md:text-base leading-none text-[#3c3c3c] select-none hover:text-[#797979] data-[state=active]:border-b-[2px] data-[state=active]:border-[#e2574c] data-[state=active]:focus:relative outline-none cursor-default"
          value={tab.value}
        >
          {tab.title}
        </Tabs.Trigger>
    ...
    {tabContent.map((content) => (
      <Tabs.Content key={content.id} className="" value={content.value}>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 w-full mb-12">
          {cards
            .filter((cardFilter) =>
              cardFilter.city.match(new RegExp(`${content.regex}`, "gi"))
            )
            .map((card) => (
